...
class hgrnet(nn.Module):
    def __init__(self, nChan, pre_Chan, nClass, pre_Class, num_feat=16, dilatability=16):
        super(hgrnet, self).__init__()
        self.cross_channel_conv = nn.Conv1d(nChan, pre_Chan, 1, bias=False)
        if pre_Chan == 256:
            self.dim_change = nn.Conv1d(pre_Chan, 16, 1, bias=False)
            pre_Chan = 16
        ...
        self.SCB = nn.Sequential(
            Conv2dWithConstraint(num_feat*2, num_feat*dilatability, (pre_Chan, 1), groups=num_feat,
        ...
        self.adapter = GCAdapter(dim=256, num_blocks=2)
        self.fc = nn.Sequential(
            LinearWithConstraint(4096, pre_Class, max_norm=0.5, doWeightNorm=True),
        )
        ...
        for param in self.fc_change.parameters():
            param.requires_grad = True
        for param in self.cross_channel_conv.parameters():
            param.requires_grad = True
...
def gdadapter_inter_device(args, total_data):
    ...
    model = hgrnet(nChan=new_channel, pre_Chan=pre_channel, nClass=trans_classes, pre_Class=pre_class).float().to(device)
